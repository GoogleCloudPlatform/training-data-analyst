...
One subtle difference is in the above map from the previous map of the whole US is that we've taken the square of the natural logarithm LN() of the number of cases. BigQuery GeoViz does not offer opacity shading with logarithmic interpolation, but we can put the logarithm in the query and then use linear interpolation in GeoViz to achieve the same effect. LN() by itself compresses the orders of magnitude a little too much, but squaring it seems about right. It's also possible to manually set up a logarithmic perception color map in GeoViz by adding buckets to the Domain fields as shown in [this blog post](https://medium.com/google-cloud/analyzing-covid-19-with-bigquery-13701a3a785).  
