// Test workflow for Pytorch mnist.
//
local env = std.extVar("__ksonnet/environments");
local overrides = std.extVar("__ksonnet/params").components.pytorch_mnist;
...
local modelDir = "gs://" + params.modelBucket + "/pytorch_mnist/models/" + prowDict["BUILD_ID"];
...
local dagTemplates = [
  ...
  },  // create-pr-symlink
  {
    ...
    template: buildTemplate {
      ...
      workingDir: srcDir + "/pytorch_mnist",
    },
    dependencies: ["checkout"],
  }, // build-images
  {
  ...
  }, // create-namespace
  {
    ...
    template: buildTemplate {
      ...
      pythonPath: OperatorPy + ":" + kubeflowTestingPy,
  ...
  },  // deploy-test
  {
    template: buildTemplate {
      ...
      command: [        
        ...
        "--namespace=" + testNamespace,
        "--service=pytorch_mnist-test-" + prowDict["BUILD_ID"],
      ],
      workingDir: srcDir + "/pytorch_mnist/testing",
    },
