...
def cnn_model(hparams):
    ...
    model.add(
        tf.keras.layers.Conv1D(filters=N_INPUTS // 2,
                               kernel_size=3,
                               padding="same",
...
def rnn_model(hparams):
    ...
    model.add(tf.keras.layers.Dense(units=N_INPUTS // 2,
                                    activation=tf.nn.relu))
    model.add(tf.keras.layers.Dense(units=1, activation=None))
    return model
...
def rnn2_model(hparams):
    ...
    model.add(
        tf.keras.layers.Dense(units=(N_INPUTS // 2) // 2,
                              activation=tf.nn.relu))
    model.add(tf.keras.layers.Dense(units=1, activation=None))
    return model
...
def serving_input_fn():
    feature_placeholders = {
        TIMESERIES_COL: tf.placeholder(dtype=tf.float32,
                                       shape=[None, N_INPUTS])
    }
    ...
    features[TIMESERIES_COL] = tf.squeeze(input=features[TIMESERIES_COL],
                                          axis=2)
...
def train_and_evaluate(output_dir, hparams):
    ...
    eval_spec = tf.estimator.EvalSpec(
        input_fn=read_dataset(filename=hparams['eval_data_path'],
