...
def to_csv(rowdict):
    ...
    for result in [no_ultrasound, w_ultrasound]:
        data = ','.join(
            [str(result[k]) if k in result else 'None' for k in CSV_COLUMNS])
        key = hashlib.sha224(
            data).hexdigest()  # hash the columns to form a key
...
def preprocess(in_test_mode, PROJECT, BUCKET, start_year):
    ...
    """.format(start_year)
        ...
        for step in ['train', 'eval']:
            if step == 'train':
                selquery = 'SELECT * FROM ({}) WHERE MOD(ABS(hashmonth),4) < 3'.format(
                    query)
            else:
                selquery = 'SELECT * FROM ({}) WHERE MOD(ABS(hashmonth),4) = 3'.format(
                    query)
            ...
            (p
             | '{}_read'.format(step) >> beam.io.Read(
                 beam.io.BigQuerySource(query=selquery, use_standard_sql=True))
             | '{}_csv'.format(step) >> beam.FlatMap(to_csv)
             | '{}_out'.format(step) >> beam.io.Write(
                 beam.io.WriteToText(
                     os.path.join(OUTPUT_DIR, '{}_{}.csv'.format(
                         step, start_year)))))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='2000',
        help=
        'Year to start extracting data. If 2000, older data will be removed')
    parser.add_argument(
        '--mode',
        choices=['local', 'cloud'],
    ...
    preprocess(args.mode == 'local', args.project, args.bucket,
               args.start_year)
