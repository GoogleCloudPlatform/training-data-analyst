...
def rnn_multistation_sampling_temperature_sequencer(filenames,
                                                    resample_by=1,
                                                    batch_size=sys.maxsize,
                                                    sequence_size=sys.maxsize,
                                                    n_forward=0,
                                                    nb_epochs=1,
                                                    tminmax=False,
                                                    keepinmem=True):
            ...
            if len(filebatch) == batchlen:
                if filecount in loaded_samples:
                    ...
                    perm = np.random.permutation(
                        loaded_samples[filecount].shape[1])
                    ...
                    for filename in filebatch:
                        with tf.gfile.Open(filename, mode='rb') as f:
                            ...
                            temperatures = np.stack(
                                [
                                    temperatures[:]['f1'],  # min temperatures
                                    temperatures[:]['f2'],  # max temperatures
                                    temperatures[:]['f3']
                                ],  # interpolated
                                axis=1)  # shape [18262, 3]
                            # Resample temperatures by averaging them across RESAMPLE_BY days
                            temperatures = np.reshape(
                                adjust(temperatures, resample_by),
                                [-1, resample_by, 3])  # [n, RESAMPLE_BY, 3]
                            temperatures = np.mean(temperatures,
                                                   axis=1)  # shape [n, 3]
                            # Shift temperature sequence to generate training targets
                            temp_targets = temperatures[n_forward:]
                            temperatures = temperatures[:temperatures.shape[
                                0] - n_forward]  # to allow n_forward=0
                            # Group temperatures into sequences of SEQLEN values
                            nseq = min(
                                sequence_size, temperatures.shape[0]
                            )  # If not even full sequence, return everything
                            temp_targets = np.reshape(
                                adjust(temp_targets, nseq),
                                [-1, nseq, 3])  # [p, SEQLEN, 3]
                            temperatures = np.reshape(
                                adjust(temperatures, nseq),
                                [-1, nseq, 3])  # [p, SEQLEN, 3]
                            # do the same with dates, assume all dates identical in all files
                            dates = np.reshape(adjust(dates, resample_by),
                                               [-1, resample_by])
                            dates = dates[:dates.shape[0] - n_forward,
                                          0]  # to allow n_forward=0
                            dates = np.reshape(adjust(dates, nseq),
                                               [-1, nseq])  # shape [p, SEQLEN]
                    ...
                    samples = np.stack(
                        samples, axis=1)  # shape [p, BATCHSIZE, SEQLEN, 3]
                    targets = np.stack(
                        targets, axis=1)  # shape [p, BATCHSIZE, SEQLEN, 3]
                    # keep them in memory
                    if keepinmem:
...
def rnn_minibatch_sequencer(data, batch_size, sequence_size, nb_epochs):
    ...
    for epoch in range(whole_epochs + 1):
        for batch in range(
                nb_batches if epoch < whole_epochs else last_nb_batch):
