...
class Recommendations(object):
  ...
  """
    ...
    """
        ...
        model_files = [
            ROW_MODEL_FILE, COL_MODEL_FILE, USER_MODEL_FILE, ITEM_MODEL_FILE,
            USER_ITEM_DATA_FILE
        ]
        for model_file in model_files:
            blob = bucket.blob(model_file)
            with open(os.path.join(local_model_path, model_file),
                      'wb') as file_obj:
        ...
        self.user_factor = np.load(
            os.path.join(local_model_path, ROW_MODEL_FILE))
        self.item_factor = np.load(
            os.path.join(local_model_path, COL_MODEL_FILE))
        self.user_map = np.load(os.path.join(local_model_path,
                                             USER_MODEL_FILE))
        self.item_map = np.load(os.path.join(local_model_path,
                                             ITEM_MODEL_FILE))
    ...
    """
        ...
        if user_idx:
            ...
            already_rated_idx = [
                np.searchsorted(self.item_map, i) for i in already_rated
            ]
            ...
            article_recommendations = [
                self.item_map[i] for i in recommendations
            ]
