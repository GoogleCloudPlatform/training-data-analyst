...
def _ratings_train_and_test(use_headers, delimiter, input_file):
  ...
  """
    ...
    tr_sparse, test_sparse = _create_sparse_train_and_test(
...
def _page_views_train_and_test(input_file):
  ...
  """
    ...
    df_user_items = views_df.groupby(['clientId',
                                      'contentId']).agg({'timeOnPage': 'sum'})
...
def train_model(args, tr_sparse):
  ...
  """
    ...
    input_tensor, row_factor, col_factor, model = wals.wals_model(
        tr_sparse, dim, reg, unobs, args['weights'], wt_type, feature_wt_exp,
        obs_wt)
