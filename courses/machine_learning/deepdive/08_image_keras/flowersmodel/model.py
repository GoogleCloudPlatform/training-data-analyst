...
def linear_model(input_tensor, hparams):
    ...
    model.add(
        tf.keras.layers.InputLayer(input_tensor=input_tensor, name="image"))
...
def cnn_model(input_tensor, hparams):
    ...
    model.add(
        tf.keras.layers.InputLayer(input_tensor=input_tensor,
                                   name="image"))  # shape = (?, 28, 28, 1)
    model.add(
        tf.keras.layers.Conv2D(
            filters=nfil1,
            kernel_size=ksize1,
            padding="same",
            activation=tf.nn.relu))  # shape = (?, 28, 28, nfil1)
    model.add(tf.keras.layers.MaxPooling2D(
        pool_size=2, strides=2))  # shape = (?, 14, 14, nfil1)
    model.add(
        tf.keras.layers.Conv2D(
            filters=nfil2,
            kernel_size=ksize2,
            padding="same",
            activation=tf.nn.relu))  # shape = (?, 14, 14, nfil2)
    model.add(tf.keras.layers.MaxPooling2D(
        pool_size=2, strides=2))  # shape = (?, 7, 7, nfil2)
    model.add(tf.keras.layers.Flatten())
...
def serving_input_fn():
    ...
    return tf.estimator.export.ServingInputReceiver(
        features=image, receiver_tensors=feature_placeholders)
...
def make_input_fn(csv_of_filenames, batch_size, mode, augment=False):
    def _input_fn():
        ...
        dataset = tf.data.TextLineDataset(filenames=csv_of_filenames).map(
            map_func=decode_csv)
...
def model_fn(features, labels, mode, params):
    ...
    class_str = tf.gather(params=LIST_OF_LABELS,
                          indices=tf.cast(x=class_int, dtype=tf.int32))
    ...
    if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            eval_metric_ops = {
                'accuracy':
                tf.metrics.accuracy(labels=labels, predictions=class_int)
            }
    else:
        loss = None
        train_op = None
