...
def read_and_preprocess(example_data):
    ...
    image = tf.image.resize_bilinear(image, [HEIGHT + 10, WIDTH + 10],
                                     align_corners=False)
...
def make_input_fn(pattern, mode, num_cores=8, transpose_input=False):
    ...
    def _input_fn(params):
        ...
        # augment and batch
        dataset = dataset.apply(
            tf.contrib.data.map_and_batch(read_and_preprocess,
...
def image_classifier(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
        loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(logits=ylogits,
                                                       labels=tf.one_hot(
                                                           labels, nclasses)))
...
def train_and_evaluate(output_dir, hparams):
    ...
    estimator.export_savedmodel(export_dir_base=os.path.join(
