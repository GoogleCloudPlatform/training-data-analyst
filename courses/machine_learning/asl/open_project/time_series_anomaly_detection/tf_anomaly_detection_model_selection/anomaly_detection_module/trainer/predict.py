...
def flag_anomalies_by_thresholding(cur_batch_size, mahalanobis_dist,
                                   anom_thresh_var):
  ...
  """
    anom_flags = tf.where(condition=tf.reduce_any(input_tensor=tf.greater(
        x=tf.abs(x=mahalanobis_dist), y=anom_thresh_var),
                                                  axis=1),
...
def anomaly_detection_predictions(cur_batch_size, seq_len, num_feat,
