...
def fix_shape_and_type_for_serving(placeholder):
  ...
  """
    ...
    split_string = tf.stack(
        values=tf.map_fn(fn=lambda x: tf.string_split(source=[placeholder[x]],
                                                      delimiter=";").values,
                         elems=tf.range(start=0,
    ...
    # shape = (batch_size, seq_len)
    feature_tensor = tf.string_to_number(string_tensor=split_string,
                                         out_type=tf.float64)
...
def serving_input_fn(feat_names, seq_len):
  ...
  """
    ...
    feature_placeholders = {
        feature: tf.placeholder(dtype=tf.string, shape=[None])
