...
if __name__ == "__main__":
    ...
    parser.add_argument("--feat_names",
                        help="Names of features.",
                        type=str,
                        required=True)
    parser.add_argument("--feat_defaults",
    ...
    parser.add_argument("--train_examples",
                        ...
                        type=int,
                        default=1024)
    parser.add_argument("--eval_examples",
                        help="Number of examples in train file.",
                        type=int,
                        default=1024)
    parser.add_argument(
        "--learning_rate",
        help=
        "How quickly or slowly we train our model by scaling the gradient.",
        type=float,
        default=0.1)
    parser.add_argument(
        "--start_delay_secs",
    ...
    ## Dense Autoencoder
    parser.add_argument("--enc_dnn_hidden_units",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8)
    parser.add_argument("--dec_dnn_hidden_units",
    ...
    parser.add_argument("--time_loss_weight",
                        ...
                        type=float,
                        default=1.0)
    parser.add_argument("--feat_loss_weight",
    ...
    parser.add_argument(
        ...
        type=str,
        default="True")
    parser.add_argument("--enc_lstm_hidden_units",
                        help="Hidden layer sizes to use for LSTM encoder.",
                        type=str,
                        default="64,32,16")
    parser.add_argument("--dec_lstm_hidden_units",
                        help="Hidden layer sizes to use for LSTM decoder.",
                        type=str,
                        default="16,32,64")
    parser.add_argument("--lstm_dropout_output_keep_probs",
    ...
    parser.add_argument(
        ...
        help=
        "Top time k principal components to keep after eigendecomposition.",
        type=int,
        default=None)
    parser.add_argument(
        "--k_principal_components_feat",
        help=
        "Top feat k principal components to keep after eigendecomposition.",
        type=int,
        default=None)
    ...
    parser.add_argument(
        "--feat_thresh_scl",
        help=
        "Max num of std devs for feature mahalanobis distance to be normal.",
        type=float,
        default=2.0)
    parser.add_argument("--time_anom_thresh",
                        help="Anomaly threshold in time dimension.",
                        type=float,
                        default=None)
    parser.add_argument("--feat_anom_thresh",
    ...
    parser.add_argument(
        "--eps",
        help=
        "Added to the cov matrix before inversion to avoid being singular.",
        type=str,
        default="1e-12")
    parser.add_argument("--f_score_beta",
    ...
    # Fix booleans
    if arguments["reverse_labels_sequence"].lower() in ("yes", "true", "t",
                                                        "y", "1"):
        arguments["reverse_labels_sequence"] = True
    else:
        arguments["reverse_labels_sequence"] = False

    if arguments["autotune_principal_components"].lower() in ("yes", "true",
                                                              "t", "y", "1"):
        arguments["autotune_principal_components"] = True
    else:
        arguments["autotune_principal_components"] = False

    if arguments["labeled_tune_thresh"].lower() in ("yes", "true", "t", "y",
                                                    "1"):
    ...
    arguments["feat_defaults"] = [
        [item] for item in arguments["feat_defaults"].split(",")
    ]
