...
def dense_encoder(X, params):
  ...
  """
    ...
    latent_matrix = tf.layers.dense(inputs=network,
...
def dense_decoder(latent_matrix, orig_dims, params):
  ...
  """
    ...
    output_matrix = tf.layers.dense(inputs=network,
                                    units=orig_dims,
                                    activation=tf.nn.relu)
...
def dense_autoencoder_model(X, mode, params, cur_batch_size, dummy_var):
  ...
  """
    ...
    if (mode == tf.estimator.ModeKeys.TRAIN
        ...
        X_time_recon_3d = tf.reshape(
            tensor=X_time_recon,
            shape=[cur_batch_size, params["seq_len"], params["num_feat"]])
        X_feat_recon_3d = tf.transpose(a=tf.reshape(
            tensor=X_feat_recon,
            shape=[cur_batch_size, params["num_feat"], params["seq_len"]]),
                                       perm=[0, 2, 1])
