...
def tune_anomaly_thresholds_unsupervised_training(
        cur_batch_size, time_anom_thresh_var, mahalanobis_dist_time,
        count_thresh_time_var, mean_thresh_time_var, var_thresh_time_var,
        feat_anom_thresh_var, mahalanobis_dist_feat, count_thresh_feat_var,
        mean_thresh_feat_var, var_thresh_feat_var, params, dummy_var):
  ...
  """
    with tf.variable_scope(name_or_scope="mahalanobis_dist_thresh_vars",
        ...
        var_time_var, mean_time_var, count_time_var = tf.cond(
            ...
            true_fn=lambda: singleton_batch_var_variable_updating(
                params["seq_len"
                       ], mahalanobis_dist_time_flat, count_thresh_time_var,
                mean_thresh_time_var, var_thresh_time_var),
            false_fn=lambda: non_singleton_batch_var_variable_updating(
                cur_batch_size, params["seq_len"], mahalanobis_dist_time_flat,
        ...
        var_feat_var, mean_feat_var, count_feat_var = tf.cond(
            ...
            true_fn=lambda: singleton_batch_var_variable_updating(
                params["num_feat"
                       ], mahalanobis_dist_feat_flat, count_thresh_feat_var,
                mean_thresh_feat_var, var_thresh_feat_var),
            false_fn=lambda: non_singleton_batch_var_variable_updating(
                cur_batch_size, params["num_feat"], mahalanobis_dist_feat_flat,
    ...
    # contexts.
    with tf.control_dependencies(control_inputs=[var_time_var, var_feat_var]):
        with tf.control_dependencies(
            ...
            with tf.control_dependencies(
                ...
                with tf.control_dependencies(control_inputs=[
                        tf.assign(ref=time_anom_thresh_var, value=time_out),
                        tf.assign(ref=feat_anom_thresh_var, value=feat_out)
                ]):
...
def tune_anomaly_thresholds_unsupervised_eval(cur_batch_size,
  ...
  """
    ...
    time_anom_flags = flag_anomalies_by_thresholding(cur_batch_size,
                                                     mahalanobis_dist_time,
                                                     time_anom_thresh_var)

    # shape = (cur_batch_size,)
    feat_anom_flags = flag_anomalies_by_thresholding(cur_batch_size,
                                                     mahalanobis_dist_feat,
                                                     feat_anom_thresh_var)
