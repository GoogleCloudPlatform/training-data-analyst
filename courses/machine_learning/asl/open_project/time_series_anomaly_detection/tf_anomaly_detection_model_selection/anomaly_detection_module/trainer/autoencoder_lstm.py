...
def create_LSTM_stack(lstm_hidden_units, lstm_dropout_output_keep_probs):
  ...
  """
    ...
    stacked_lstm_cells = tf.contrib.rnn.MultiRNNCell(cells=dropout_lstm_cells,
                                                     state_is_tuple=True)
...
def rnn_decoder(dec_input, init_state, cell, infer, dnn_hidden_units,
                num_feat):
        ...
        for index, decoder_input in enumerate(dec_input):
            ...
            if previous_output is not None:
                ...
                with tf.variable_scope(name_or_scope="dnn",
                    ...
                    logits = tf.layers.dense(inputs=network,
                                             units=num_feat,
                                             activation=None)
...
def lstm_enc_dec_autoencoder_model(X, mode, params, cur_batch_size, dummy_var):
  ...
  """
    ...
    if params["reverse_labels_sequence"]:
        Y = tf.reverse_sequence(
            input=X,
            seq_lengths=tf.tile(input=tf.constant(value=[params["seq_len"]],
                                                  dtype=tf.int64),
                                multiples=tf.expand_dims(input=cur_batch_size,
                                                         axis=0)),
            seq_axis=1,
            batch_axis=0)
    else:
    ...
    with tf.variable_scope("encoder"):
        ...
        _, enc_states = tf.nn.static_rnn(
            ...
            inputs=X_sequence,
            initial_state=enc_stacked_lstm_cells.zero_state(batch_size=tf.cast(
                x=cur_batch_size, dtype=tf.int32),
                                                            dtype=tf.float64),
            dtype=tf.float64)
    ...
    if (mode == tf.estimator.ModeKeys.TRAIN
        ...
        fake_labels = [
            tf.zeros(shape=[cur_batch_size, params["num_feat"]],
                     dtype=tf.float64) for _ in range(params["seq_len"])
        ]
    ...
    if (mode == tf.estimator.ModeKeys.TRAIN
        ...
        if params["reverse_labels_sequence"]:
            ...
            predictions = tf.reverse_sequence(
                input=predictions,
                seq_lengths=tf.tile(
                    input=tf.constant(value=[params["seq_len"]],
                                      dtype=tf.int64),
