...
def read_dataset_frame(file_pattern, mode, batch_size):
    def _input_fn():
        print("\nread_dataset_frame: _input_fn: file_pattern = {}".format(
            file_pattern))
        print("read_dataset_frame: _input_fn: mode = {}".format(mode))
        print("read_dataset_frame: _input_fn: batch_size = {}".format(
            batch_size))
        ...
        def decode_example(serialized_examples):
            ...
            print(
                "read_dataset_frame: _input_fn: decode_example: contexts = {}".
                format(contexts)
            )  # shape = video_id = (), labels = SparseTensor object
            print(
                "read_dataset_frame: _input_fn: decode_example: features = {}".
                format(features)
            ...
            print(
                "read_dataset_frame: _input_fn: decode_example: decoded_rgb = {}"
                .format(decoded_rgb))
            rgb_matrix = resize_axis(
                tensor=dequantize(decoded_rgb), axis=0,
                new_size=MAX_FRAMES)  # shape = (MAX_FRAMES, 1024)
            print(
                "read_dataset_frame: _input_fn: decode_example: rgb_matrix = {}"
                .format(rgb_matrix))
            ...
            print(
                "read_dataset_frame: _input_fn: decode_example: decoded_audio = {}"
            ...
            print(
                "read_dataset_frame: _input_fn: decode_example: audio_matrix = {}"
                .format(audio_matrix))
            ...
            labels = tf.cast(x=tf.sparse_to_dense(
                ...
                validate_indices=False),
                             dtype=tf.float32)
            print(
                "read_dataset_frame: _input_fn: decode_example: labels = {}\n".
        ...
        print("read_dataset_frame: _input_fn: dataset.TFRecordDataset = {}".
              format(dataset))
        ...
        print(
            "read_dataset_frame: _input_fn: dataset.map = {}".format(dataset))
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            print("read_dataset_frame: _input_fn: dataset.shuffle = {}".format(
                dataset))
        else:
        ...
        print("read_dataset_frame: _input_fn: dataset.repeat = {}".format(
            dataset))

        # Group the data into batches
        dataset = dataset.batch(batch_size=batch_size)
        print("read_dataset_frame: _input_fn: dataset.batch = {}".format(
            dataset))
        ...
        print("read_dataset_frame: _input_fn: batch_features = {}".format(
            batch_features))
...
def frame_level_model(features, labels, mode, params):
    print(
        "\nframe_level_model: features = {}".format(features)
    )  # features['rgb'].shape = (batch_size, MAX_FRAMES, 1024), features['audio'].shape = (batch_size, MAX_FRAMES, 128)
    ...
    print("frame_level_model: current_batch_size = {}".format(
        current_batch_size))
    ...
    print(
        "frame_level_model: combined_features = {}".format(combined_features))

    # Reshape the combined features into a 2-D tensor that we will pass through our DNN
    reshaped_combined_features = tf.reshape(
        tensor=combined_features,
        shape=[current_batch_size * MAX_FRAMES, 1024 + 128
               ])  # shape = (current_batch_size * MAX_FRAMES, 1024 + 128)
    print("frame_level_model: reshaped_combined_features = {}".format(
        reshaped_combined_features))
    ...
    print(
        "frame_level_model: network = reshaped_combined_features = {}".format(
            network))
    ...
    for units in params['hidden_units']:
        ...
        print("frame_level_model: network = {}, units = {}".format(
            network, units))

    # Connect the final hidden layer to a dense layer with no activation to get the logits
    logits = tf.layers.dense(
        inputs=network, units=NUM_CLASSES, activation=None
    ...
    probabilities = tf.sigmoid(
        logits)  # shape = (current_batch_size * MAX_FRAMES, NUM_CLASSES)
    ...
    reshaped_probabilities = tf.reshape(
        tensor=probabilities,
        shape=[current_batch_size, MAX_FRAMES, NUM_CLASSES
               ])  # shape = (current_batch_size, MAX_FRAMES, NUM_CLASSES)
    print("frame_level_model: reshaped_probabilities = {}".format(
...
def train_and_evaluate(args):
    ...
    estimator = tf.estimator.Estimator(model_fn=frame_level_model,
    ...
    train_spec = tf.estimator.TrainSpec(input_fn=read_dataset_frame(
    ...
    eval_spec = tf.estimator.EvalSpec(
        input_fn=read_dataset_frame(file_pattern=args['eval_file_pattern'],
