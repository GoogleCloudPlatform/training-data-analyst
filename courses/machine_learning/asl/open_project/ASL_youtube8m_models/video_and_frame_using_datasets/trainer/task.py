...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--video_hidden_units',
        help = 'The number of video level hidden units per hidden layer',
        default = "1024 256 64",
        type = str
    )
    parser.add_argument(
        '--frame_hidden_units',
        help = 'The number of frame level hidden units per hidden layer',
        default = "1024 256 64",
        type = str
    )
    parser.add_argument(
    ...
    arguments['video_hidden_units'] = [int(x) for x in arguments['video_hidden_units'].split(' ')]
    arguments['frame_hidden_units'] = [int(x) for x in arguments['frame_hidden_units'].split(' ')]
