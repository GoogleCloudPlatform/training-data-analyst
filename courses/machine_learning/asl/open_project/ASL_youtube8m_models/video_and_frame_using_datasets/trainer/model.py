...
def read_dataset_video_and_frame(file_pattern, mode, batch_size):
    def _input_fn():
        print("\nread_dataset_video_and_frame: _input_fn: file_pattern = {}".
              format(file_pattern))
        print(
            "read_dataset_video_and_frame: _input_fn: mode = {}".format(mode))
        print(
            "read_dataset_video_and_frame: _input_fn: batch_size = {}".format(
                batch_size))
        ...
        def dequantize(feat_vector,
            ...
            ) + min_quantized_value  # create bias term to shift our scaled feature vector
        ...
        def resize_axis(tensor, axis, new_size, fill_value=0):
            tensor = tf.convert_to_tensor(
                value=tensor)  # ensure tensor is a tensor
            shape = tf.unstack(
                value=tf.shape(input=tensor)
            )  # create a list where each element is a 1-D tensor the size of each dimension in tensor

            pad_shape = shape[:]  # create a copy of the shape list of 1-D tensors
            pad_shape[axis] = tf.maximum(
                x=0, y=new_size - shape[axis]
            )  # change the size of the axis dimension to the maximum of 0 and the new size of our padded shape

            shape[axis] = tf.minimum(
                x=shape[axis], y=new_size
            )  # change the size of the axis dimension to the minimum of our original shape and the new size of our padded shape
            shape = tf.stack(
                values=shape
            )  # stack the list of tensor sizes back into a larger tensor

            resized = tf.concat(
                values=[
                    tf.slice(
                        input_=tensor,
                        begin=tf.zeros_like(tensor=shape),
                        size=shape
                    ),  # slice the tensor starting at the 0th index in each dimension and going as far as our adjusted shape in each dimension
                    tf.fill(
                        dims=tf.stack(values=pad_shape),
                        value=tf.cast(x=fill_value, dtype=tensor.dtype)
                    )  # fill the rest of the tensor with the fill value
                ],
                axis=axis
            )  # concatenate our sliced tensor with our fill value tensor together

            new_shape = tensor.get_shape().as_list(
            )  # get the static shape of the tensor and output it to a list
            new_shape[
                axis] = new_size  # change the static shape's axis to our new size
            resized.set_shape(
                shape=new_shape
            )  # set the static shape of our resized tensor to our new shape
        ...
        def video_decode_example(serialized_examples):
            ...
            print(
                "\nread_dataset_video_and_frame: _input_fn: video_decode_example: features = {}"
                .format(features)
            ...
            print(
                "read_dataset_video_and_frame: _input_fn: video_decode_example: sparse_labels = {}"
            ...
            labels = tf.cast(x=tf.sparse_to_dense(
                ...
                validate_indices=False),
                             dtype=tf.float32)
            print(
                "read_dataset_video_and_frame: _input_fn: video_decode_example: labels = {}\n"
        ...
        def frame_decode_example(serialized_examples):
            ...
            print(
                "\nread_dataset_video_and_frame: _input_fn: frame_decode_example: contexts = {}"
                .format(contexts)
            )  # shape = video_id = (), labels = SparseTensor object
            print(
                "read_dataset_video_and_frame: _input_fn: frame_decode_example: features = {}"
                .format(features)
            )  # shape = rgb = (frames_per_video,), audio = (frames_per_video,)

            # Create features
            # Pass video_id to features
            features['video_id'] = contexts[
                'video_id']  # shape = video_id = (), rgb = (frames_per_video,), audio = (frames_per_video,)
            print(
                "read_dataset_video_and_frame: _input_fn: frame_decode_example: features = {}"
                .format(features))

            # Fix rgb data
            decoded_rgb = tf.reshape(
                tensor=tf.cast(x=tf.decode_raw(bytes=features["rgb"],
                                               out_type=tf.uint8),
                               dtype=tf.float32),
                shape=[-1, 1024])  # shape = (frames_per_video, 1024)
            print(
                "read_dataset_video_and_frame: _input_fn: frame_decode_example: decoded_rgb = {}"
                .format(decoded_rgb))
            rgb_matrix = resize_axis(
                tensor=dequantize(decoded_rgb), axis=0,
                new_size=MAX_FRAMES)  # shape = (MAX_FRAMES, 1024)
            print(
                "read_dataset_video_and_frame: _input_fn: frame_decode_example: rgb_matrix = {}"
                .format(rgb_matrix))
            features['rgb'] = rgb_matrix
            print(
                "read_dataset_video_and_frame: _input_fn: frame_decode_example: features = {}"
                .format(features)
            )  # shape = video_id = (), rgb = (MAX_FRAMES, 1024), audio = (frames_per_video,)

            # Fix audio data
            decoded_audio = tf.reshape(
                tensor=tf.cast(x=tf.decode_raw(bytes=features["audio"],
                                               out_type=tf.uint8),
                               dtype=tf.float32),
                shape=[-1, 128])  # shape = (frames_per_video, 128)
            print(
                "read_dataset_video_and_frame: _input_fn: frame_decode_example: decoded_audio = {}"
                .format(decoded_audio))
            audio_matrix = resize_axis(
                tensor=dequantize(decoded_audio), axis=0,
                new_size=MAX_FRAMES)  # shape = (MAX_FRAMES, 128)
            print(
                "read_dataset_video_and_frame: _input_fn: frame_decode_example: audio_matrix = {}"
                .format(audio_matrix))
            features['audio'] = audio_matrix
            print(
                "read_dataset_video_and_frame: _input_fn: frame_decode_example: features = {}"
                .format(features)
            )  # shape = video_id = (), rgb = (MAX_FRAMES, 1024), audio = (MAX_FRAMES, 128)

            # Extract and format labels
            labels = tf.cast(x=tf.sparse_to_dense(
                sparse_indices=contexts['labels'].values,
                output_shape=(NUM_CLASSES, ),
                sparse_values=1,
                validate_indices=False),
                             dtype=tf.float32)
            print(
                "read_dataset_video_and_frame: _input_fn: frame_decode_example: labels = {}\n"
                .format(labels))  # shape = (NUM_CLASSES,)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            video_file_list = tf.gfile.Glob(
                filename=file_pattern + "/video_level/train/train*.tfrecord")
            frame_file_list = tf.gfile.Glob(
                filename=file_pattern + "/frame_level/train/train*.tfrecord")
        else:
            video_file_list = tf.gfile.Glob(
                filename=file_pattern +
                "/video_level/validate/validate-0.tfrecord")
            frame_file_list = tf.gfile.Glob(
                filename=file_pattern +
                "/frame_level/validate/validate-0.tfrecord")
        ...
        print(
            "read_dataset_video_and_frame: _input_fn: video_dataset.TFRecordDataset = {}"
            .format(video_dataset))
        frame_dataset = tf.data.TFRecordDataset(filenames=frame_file_list)
        print(
            "read_dataset_video_and_frame: _input_fn: frame_dataset.TFRecordDataset = {}"
            .format(frame_dataset))

        # Decode TF Record dataset examples
        video_dataset = video_dataset.map(
            map_func=lambda x: video_decode_example(serialized_examples=x))
        print(
            "read_dataset_video_and_frame: _input_fn: video_dataset.map = {}".
            format(video_dataset))
        frame_dataset = frame_dataset.map(
            map_func=lambda x: frame_decode_example(serialized_examples=x))
        print(
            "read_dataset_video_and_frame: _input_fn: frame_dataset.map = {}".
            format(frame_dataset))

        # Zip together video and frame datasets
        combined_dataset = tf.data.Dataset.zip(datasets=(video_dataset,
                                                         frame_dataset))
        print("read_dataset_video_and_frame: _input_fn: combined_dataset = {}".
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            combined_dataset = combined_dataset.shuffle(
                buffer_size=batch_size * 10, seed=None)
            print(
                "read_dataset_video_and_frame: _input_fn: combined_dataset.shuffle = {}"
                .format(combined_dataset))
        else:
        ...
        print(
            "read_dataset_video_and_frame: _input_fn: combined_dataset.repeat = {}"
            .format(combined_dataset))

        # Group the data into batches
        combined_dataset = combined_dataset.batch(batch_size=batch_size)
        print(
            "read_dataset_video_and_frame: _input_fn: combined_dataset.batch = {}"
            .format(combined_dataset))

        # Create a iterator and then pull the next batch of features and labels from the example queue
        (video_batch_features, video_batch_labels), (
            frame_batch_features, frame_batch_labels
        ) = combined_dataset.make_one_shot_iterator().get_next()
        print(
            "read_dataset_video_and_frame: _input_fn: video_batch_features = {}"
            .format(video_batch_features))
        print(
            "read_dataset_video_and_frame: _input_fn: video_batch_labels = {}".
            format(video_batch_labels))
        print(
            "read_dataset_video_and_frame: _input_fn: frame_batch_features = {}"
            .format(frame_batch_features))
        print(
            "read_dataset_video_and_frame: _input_fn: frame_batch_labels = {}\n"
        ...
        print("read_dataset_video_and_frame: _input_fn: batch_features = {}".
              format(batch_features))

        # Only need one set of labels
        batch_labels = video_batch_labels
        print("read_dataset_video_and_frame: _input_fn: batch_labels = {}".
              format(batch_labels))
...
def video_and_frame_level_model(features, labels, mode, params):
  ...
  '''
    ...
    print("video_and_frame_level_model: current_batch_size = {}".format(
        current_batch_size))
    ...
    combined_video_features = tf.concat(
        values=[features['mean_rgb'], features['mean_audio']],
        axis=1)  # shape = (current_batch_size, 1024 + 128)
    print("video_and_frame_level_model: combined_video_features = {}".format(
    ...
    print(
        ...
        .format(video_network))

    # Add video hidden layers with the given number of units/neurons per layer
    for units in params['video_hidden_units']:
        video_network = tf.layers.dense(
            inputs=video_network, units=units,
            activation=tf.nn.relu)  # shape = (current_batch_size, units)
        print("video_and_frame_level_model: video_network = {}, units = {}".
              format(video_network, units))

    # Connect the final hidden layer to a dense layer with no activation to get a video logit for each class
    video_logits = tf.layers.dense(
        inputs=video_network, units=NUM_CLASSES,
        activation=None)  # shape = (current_batch_size, NUM_CLASSES)
    print(
        "video_and_frame_level_model: video_logits = {}".format(video_logits))

    # Find the video probabilities of all video logits
    video_probabilities = tf.sigmoid(
        video_logits)  # shape = (current_batch_size, NUM_CLASSES)
    print("video_and_frame_level_model: video_probabilities = {}".format(
        video_probabilities))
    ...
    combined_frame_features = tf.concat(
        values=[features['rgb'], features['audio']],
        axis=2)  # shape = (current_batch_size, MAX_FRAMES, 1024 + 128)
    print("video_and_frame_level_model: combined_frame_features = {}".format(
    ...
    reshaped_combined_frame_features = tf.reshape(
        tensor=combined_frame_features,
        shape=[current_batch_size * MAX_FRAMES, 1024 + 128
               ])  # shape = (current_batch_size * MAX_FRAMES, 1024 + 128)
    print("video_and_frame_level_model: reshaped_combined_frame_features = {}".
          format(reshaped_combined_frame_features))
    ...
    print(
        ...
        .format(frame_network))

    # Add frame hidden layers with the given number of units/neurons per layer
    for units in params['frame_hidden_units']:
        frame_network = tf.layers.dense(
            inputs=frame_network, units=units, activation=tf.nn.relu
        )  # shape = (current_batch_size * MAX_FRAMES, units)
        print("video_and_frame_level_model: frame_network = {}, units = {}".
              format(frame_network, units))

    # Connect the final hidden layer to a dense layer with no activation to get a frame logit for each class
    frame_logits = tf.layers.dense(
        inputs=frame_network, units=NUM_CLASSES, activation=None
    )  # shape = (current_batch_size * MAX_FRAMES, NUM_CLASSES)
    print(
        "video_and_frame_level_model: frame_logits = {}".format(frame_logits))

    # Find the frame probabilities of all frame logits using sigmoid since this is multi-class, multi-label
    frame_probabilities = tf.sigmoid(
        frame_logits)  # shape = (current_batch_size * MAX_FRAMES, NUM_CLASSES)
    print("video_and_frame_level_model: frame_probabilities = {}".format(
        frame_probabilities))

    # Reshape the frame probability tensor so that each frame has its own matrix
    reshaped_frame_probabilities = tf.reshape(
        tensor=frame_probabilities,
        shape=[current_batch_size, MAX_FRAMES, NUM_CLASSES
               ])  # shape = (current_batch_size, MAX_FRAMES, NUM_CLASSES)
    print("video_and_frame_level_model: reshaped_frame_probabilities = {}".
          format(reshaped_frame_probabilities))

    # Average the frame probabilities over all frames per each example and each class
    average_probabilities_over_frames = tf.reduce_mean(
        input_tensor=reshaped_frame_probabilities,
        axis=1)  # shape = (current_batch_size, NUM_CLASSES)
    print(
        "video_and_frame_level_model: average_probabilities_over_frames = {}".
        format(average_probabilities_over_frames))

    ################################################################################

    # Average together the video and frame probabilities
    combined_probabilities = (
        video_probabilities + average_probabilities_over_frames
    ) * 0.5  # shape = (current_batch_size, NUM_CLASSES)
    print("video_and_frame_level_model: combined_probabilities = {}".format(
        combined_probabilities))

    # Select the top k probablities in descending order
    top_k_probabilities = tf.nn.top_k(
        input=combined_probabilities, k=params['top_k'],
        sorted=True)  # shape = (current_batch_size, top_k)
    print("video_and_frame_level_model: top_k_probabilities = {}".format(
        top_k_probabilities))

    # Find the inverse of all of the average probabilities to get all of the average logits
    inverse_probabilities_logits = tf.log(
        combined_probabilities + 0.00000001) - tf.log(
            1.0 - combined_probabilities +
            0.00000001)  # shape = (current_batch_size, NUM_CLASSES)
    print("video_and_frame_level_model: inverse_probabilities_logits = {}".
          format(inverse_probabilities_logits))

    # Select the top k logits using the indices of the top k probabilities in descending order
    top_k_logits = tf.map_fn(
        fn=lambda x: tf.gather(params=inverse_probabilities_logits[x],
                               indices=top_k_probabilities.indices[x]),
    ...
    print("video_and_frame_level_model: top_k_classes = {}".format(
        top_k_classes))
    ...
    predictions = tf.where(
        condition=average_probabilities_over_frames >
    ...
    top_k_predictions = tf.where(
        condition=top_k_probabilities.values >
    ...
    if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
        ...
        loss = tf.reduce_mean(
            input_tensor=-tf.reduce_sum(input_tensor=labels * tf.log(
                x=average_probabilities_over_frames + 0.00000001),
                                        axis=1))
...
def serving_input_fn():
    ...
    # This function fixes dynamic shape ambiguity of last dimension so that we will be able to use it in our DNN (since tf.layers.dense require the last dimension to be known)
    def get_shape_and_set_modified_shape_3D(tensor,
                                            additional_dimension_sizes):
        # Get static shape for tensor and convert it to list
        shape = tensor.get_shape().as_list()
        # Set outer shape to additional_dimension_sizes[0] * additional_dimension_sizes[1] since we know that this is the correct size
        shape[
            1] = additional_dimension_sizes[0] * additional_dimension_sizes[0]
        # Set the shape of tensor to our modified shape
        tensor.set_shape(
            shape=shape
        )  # shape = (batch_size, additional_dimension_sizes[0] * additional_dimension_sizes[1])
        print(
            "serving_input_fn: get_shape_and_set_modified_shape_3D: tensor = {}, additional_dimension_sizes = {}"
            .format(tensor, additional_dimension_sizes))
        # Finally reshape tensor into the shape it is supposed to be for the model function
        tensor = tf.reshape(
            tensor=tensor,
            shape=[
                -1, additional_dimension_sizes[0],
                additional_dimension_sizes[1]
            ]
        )  # shape = (batch_size, additional_dimension_sizes[0], additional_dimension_sizes[1])
        print(
            "serving_input_fn: get_shape_and_set_modified_shape_3D: tensor = {}, additional_dimension_sizes = {}"
    ...
    features = {
        ...
        "rgb":
        fix_shape_and_type_for_serving(
            placeholder=feature_placeholders["rgb"]),
        "audio":
        fix_shape_and_type_for_serving(
            placeholder=feature_placeholders["audio"])
    }
    ...
    features["rgb"] = get_shape_and_set_modified_shape_3D(
        tensor=features["rgb"], additional_dimension_sizes=[MAX_FRAMES, 1024])
    features["audio"] = get_shape_and_set_modified_shape_3D(
        tensor=features["audio"], additional_dimension_sizes=[MAX_FRAMES, 128])
...
def train_and_evaluate(args):
    ...
    estimator = tf.estimator.Estimator(model_fn=video_and_frame_level_model,
                                       model_dir=args['output_dir'],
                                       params={
                                           'video_hidden_units':
                                           args['video_hidden_units'],
                                           'frame_hidden_units':
                                           args['frame_hidden_units'],
                                           'top_k':
                                           args['top_k']
                                       })
    # Create train spec to read in our training data
    train_spec = tf.estimator.TrainSpec(input_fn=read_dataset_video_and_frame(
