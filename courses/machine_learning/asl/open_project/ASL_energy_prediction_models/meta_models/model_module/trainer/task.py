...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_file_pattern',
                        ...
                        required=True)
    parser.add_argument('--model_fn_name',
    ...
    parser.add_argument(
        '--horizon',
        help=
        'Number of examples to skip between the input and output sequences',
        type=int,
        default=0)
    parser.add_argument('--output_sequence_length',
                        help='Number of examples to use as the labels',
                        type=int,
                        default=1)
    parser.add_argument('--reverse_sequence',
    ...
    parser.add_argument(
        '--lstm_hidden_units',
        help=
        'Hidden layer sizes to use for LSTMs -- provide space-separated layers',
        type=str,
        default="128 32 4")
    parser.add_argument(
        '--lstm_dropout_output_keep_probs',
        help=
        'Dropout output keep probabilities to use for LSTMs -- provide space-separated layers',
        type=str,
        default="0.95 1.0 1.0")
    parser.add_argument(
        '--dnn_hidden_units',
        help=
        'Hidden layer sizes to use for DNN -- provide space-separated layers',
        type=str,
        default="128 32 4")
    parser.add_argument(
        '--learning_rate',
    ...
    arguments['input_sequence_length'] = max(
        1, arguments['input_sequence_length'])
    arguments['horizon'] = max(0, arguments['horizon'])
    arguments['output_sequence_length'] = max(
        1, arguments['output_sequence_length'])
    ...
    print("Will use LSTM output keep probabilities of {}".format(
        arguments['lstm_dropout_output_keep_probs']))
