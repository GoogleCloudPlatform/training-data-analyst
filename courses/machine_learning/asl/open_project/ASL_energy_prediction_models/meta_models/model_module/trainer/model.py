...
def read_dataset(filename, mode, batch_size, params):
    def _input_fn():
        ...
        def decode_csv(value_column):
            ...
            print("read_dataset: _input_fn: decode_csv: columns = {}".format(
                columns))
            features = dict(zip(CSV_COLUMNS, columns))
            print("read_dataset: _input_fn: decode_csv: features = {}".format(
                features))
            return features
        ...
        def create_sequences(features):
            ...
            print("\nread_dataset: _input_fn: create_sequences: features = {}".
                  format(features))
            ...
            sequence_lengths = tf.tile(input=[params["input_sequence_length"]],
                                       multiples=[params["batch_size"]])
            print(
                "read_dataset: _input_fn: create_sequences: sequence_lengths = {}"
            ...
            label = features[LABEL_COLUMN][
                ...
                params['horizon']:]  # shape = (batch_size,)
            print(
                "read_dataset: _input_fn: create_sequences: label = {}".format(
                    label))

            # Now use tf.map_fn to break our labels and features into a params['batch_size'] number of sequences offset by one timestep
            label = tf.map_fn(
                fn=lambda i: label[i:params['output_sequence_length'] + i],
                elems=tf.range(params['batch_size']),
                dtype=tf.float32
            )  # shape = (params['batch_size'], params['input_sequence_length'])
            print(
                "read_dataset: _input_fn: create_sequences: label = {}".format(
                    label))

            for key, val in features.items():
                features[key] = tf.map_fn(
                    fn=lambda i: features[key][i:params['input_sequence_length'
                                                        ] + i],
                    elems=tf.range(params['batch_size']),
                    dtype=tf.float32
                )  # shape = (params['batch_size'], params['input_sequence_length'])
                ...
                if params["reverse_sequence"] == True:
                    features[key] = tf.reverse_sequence(
                        ...
                        batch_axis=0)
                    print(
                        "read_dataset: _input_fn: create_sequences: reversed features[{}] = {}"
                        .format(key, features[key]))
        ...
        print("read_dataset: _input_fn: dataset.map(decode_csv) = {}".format(
            dataset))
        ...
        print(
            "read_dataset: _input_fn: dataset.repeat(num_epochs) = {}".format(
                dataset))

        # Group the data into batches
        dataset = dataset.apply(
            transformation_func=tf.contrib.data.batch_and_drop_remainder(
                batch_size=batch_size))
        print("read_dataset: _input_fn: dataset.batch(batch_size) = {}".format(
            dataset))

        # Create sequences out of our batched data
        dataset = dataset.map(map_func=create_sequences)
        print("read_dataset: _input_fn: dataset.map(create_sequences) = {}".
              format(dataset))
        ...
        print("read_dataset: _input_fn: batch_features = {}".format(
            batch_features))
...
def dnn_regression(features, labels, mode, params):
    ...
    X = tf.stack(
        features.values(), axis=2
    )  # shape = (batch_size, input_sequence_length, number_of_features)
    print("dnn_regression: X = {}".format(X))

    # For DNN I want to flatten everything
    X_flattened = tf.reshape(X, [
        current_batch_size,
        params['input_sequence_length'] * number_of_features
    ])  # shape = (batch_size, input_sequence_length * number_of_features)
    ...
    for units in params['dnn_hidden_units']:
        network = tf.layers.dense(
            inputs=network, units=units, activation=tf.nn.relu
        )  # shape = (current_batch_size, dnn_hidden_units[i])
        print("dnn_regression: network = {}, units = {}".format(
            network, units))

    # Connect the final hidden layer to a dense layer with no activation to get the predictions
    predictions = tf.layers.dense(
        inputs=network,
        units=params['output_sequence_length'],
        activation=None
    )  # shape = (current_batch_size, output_sequence_length)
...
def stacked_lstm_regression(features, labels, mode, params):
    ...
    # Get dynamic batch size
    current_batch_size = tf.shape(features["price"])[0]
    print("stacked_lstm_regression: current_batch_size = {}".format(
        current_batch_size))

    # Get the number of features
    number_of_features = len(features)
    print("stacked_lstm_regression: number_of_features = {}".format(
        number_of_features))

    # Stack all of the features into a 3-D tensor
    X = tf.stack(
        values=features.values(), axis=2
    )  # shape = (current_batch_size, input_sequence_length, number_of_features)
    ...
    X_sequence = tf.unstack(value=X,
                            num=params['input_sequence_length'],
                            axis=1)
    ...
    lstm_cells = [
        tf.contrib.rnn.BasicLSTMCell(num_units=units,
                                     forget_bias=1.0,
                                     state_is_tuple=True)
        for units in params["lstm_hidden_units"]
    ]  # list of LSTM cells
    ...
    dropout_lstm_cells = [
        tf.nn.rnn_cell.DropoutWrapper(
            ...
            input_keep_prob=1.0,
            output_keep_prob=params["lstm_dropout_output_keep_probs"]
            [cell_index],
            state_keep_prob=1.0) for cell_index in range(len(lstm_cells))
    ]
    print("stacked_lstm_regression: dropout_lstm_cells = {}".format(
        dropout_lstm_cells))
    ...
    print("stacked_lstm_regression: stacked_lstm_cells = {}".format(
        stacked_lstm_cells))

    # Create a static RNN where we will keep a list of all of the intermediate hidden state outputs
    outputs, _ = tf.nn.static_rnn(
        cell=
        stacked_lstm_cells,  # creates list input_sequence_length elements long of 2-D tensors of shape = (current_batch_size, lstm_hidden_units[-1])
        inputs=X_sequence,
        initial_state=stacked_lstm_cells.zero_state(
            batch_size=current_batch_size, dtype=tf.float32),
        dtype=tf.float32)
    print("stacked_lstm_regression: outputs = {}".format(outputs))

    # Slice outputs to keep only the last cell of the RNN
    last_output = outputs[
        -1]  # shape = (current_batch_size, lstm_hidden_units[-1])
    ...
    for units in params['dnn_hidden_units']:
        ...
        print("stacked_lstm_regression: network = {}, units = {}".format(
            network, units))
...
def stacked_lstmN_regression(features, labels, mode, params):
    ...
    # Get dynamic batch size
    current_batch_size = tf.shape(features["price"])[0]
    print("stacked_lstmN_regression: current_batch_size = {}".format(
        current_batch_size))

    # Get the number of features
    number_of_features = len(features)
    print("stacked_lstmN_regression: number_of_features = {}".format(
        number_of_features))
    ...
    print("stacked_lstmN_regression: dropout_lstm_cells = {}".format(
        dropout_lstm_cells))
    ...
    stacked_lstm_cells = tf.contrib.rnn.MultiRNNCell(
        cells=dropout_lstm_cells,
        state_is_tuple=True)  # combines list into MultiRNNCell object
    print("stacked_lstmN_regression: stacked_lstm_cells = {}".format(
        stacked_lstm_cells))
    ...
    stacked_outputs = tf.stack(
        values=outputs, axis=1
    )  # shape = (current_batch_size, input_sequence_length, lstm_hidden_units[-1])
    print("stacked_lstmN_regression: stacked_outputs = {}".format(
        stacked_outputs))

    # Flatten stacked outputs into a 2-D tensor
    flattened_stacked_outputs = tf.reshape(
        tensor=stacked_outputs,
        shape=[
            current_batch_size,
            params['input_sequence_length'] * params['lstm_hidden_units'][-1]
        ]
    )  # shape = (current_batch_size, input_sequence_length * lstm_hidden_units[-1])
    print("stacked_lstmN_regression: flattened_stacked_outputs = {}".format(
        flattened_stacked_outputs))
    ...
    for units in params['dnn_hidden_units']:
        ...
        print("stacked_lstmN_regression: network = {}, units = {}".format(
            network, units))
...
def encoder_decoder_stacked_lstm_regression(features, labels, mode, params):
    print("\nencoder_decoder_stacked_lstm_regression: features = {}".format(
        features))
    print("encoder_decoder_stacked_lstm_regression: labels = {}".format(
        labels))  # shape = (current_batch_size, output_sequence_length)
    print("encoder_decoder_stacked_lstm_regression: mode = {}".format(mode))
    print(
        "encoder_decoder_stacked_lstm_regression: params = {}".format(params))

    # 0. Get input sequence tensor into correct shape
    # Get dynamic batch size in case there was a partially filled batch
    current_batch_size = tf.shape(features["price"])[0]
    print("encoder_decoder_stacked_lstm_regression: current_batch_size = {}".
          format(current_batch_size))

    # Get the number of features
    number_of_features = len(features)
    print("encoder_decoder_stacked_lstm_regression: number_of_features = {}".
    ...
    X_sequence = tf.unstack(value=X,
                            num=params['input_sequence_length'],
                            axis=1)
    print("encoder_decoder_stacked_lstm_regression: X_sequence = {}".format(
        X_sequence))
    ...
    print("encoder_decoder_stacked_lstm_regression: lstm_cells = {}".format(
        lstm_cells))
    ...
    print("encoder_decoder_stacked_lstm_regression: dropout_lstm_cells = {}".
    ...
    stacked_lstm_cells = tf.contrib.rnn.MultiRNNCell(
        cells=dropout_lstm_cells,
        state_is_tuple=True)  # combines list into MultiRNNCell object
    print("encoder_decoder_stacked_lstm_regression: stacked_lstm_cells = {}".
    ...
    with tf.variable_scope("encoder"):
        ...
        print("encoder_decoder_stacked_lstm_regression: encoder_cells = {}".
              format(encoder_cells))

        # Encode the input sequence using our encoder stack of LSTMs
        encoder_outputs, encoder_final_state = tf.nn.static_rnn(
            cell=encoder_cells, inputs=X_sequence, dtype=tf.float32)
        print(
            "encoder_decoder_stacked_lstm_regression: encoder_outputs = {}".
            format(encoder_outputs)
        )  # list input_sequence_length long of shape = (current_batch_size, lstm_hidden_units[-1])
        print(
            "encoder_decoder_stacked_lstm_regression: encoder_final_state = {}"
            .format(encoder_final_state
                    ))  # tuple of final encoder c_state and h_state
...
# Starts with an intial state of the final encoder states
    ...
    def rnn_decoder(decoder_inputs, initial_state, cell, inference):
        ...
        with tf.variable_scope("decoder"):
            ...
            print(
                "encoder_decoder_stacked_lstm_regression: rnn_decoder: state = {}"
                .format(state))
            ...
            for index, decoder_input in enumerate(decoder_inputs):
                ...
                if previous_output is not None:
                    ...
                    print(
                        "encoder_decoder_stacked_lstm_regression: rnn_decoder: network = {}"
                        .format(network))
                    ...
                    with tf.variable_scope(name_or_scope="dnn",
                        ...
                        for units in params['dnn_hidden_units']:
                            ...
                            print(
                                "encoder_decoder_stacked_lstm_regression: rnn_decoder: network = {}, units = {}"
                                .format(network, units))

                        # Connect the final hidden layer to a dense layer with no activation to get the logits
                        logits = tf.layers.dense(
                            inputs=network, units=1,
                            activation=None)  # shape = (current_batch_size, 1)
                        print(
                            "encoder_decoder_stacked_lstm_regression: rnn_decoder: logits = {}\n"
                            .format(logits))
                    ...
                    print(
                        "encoder_decoder_stacked_lstm_regression: rnn_decoder: decoder_input = {}\n"
                ...
                print(
                    "encoder_decoder_stacked_lstm_regression: rnn_decoder: output = {}"
                    .format(output)
                )  # shape = (current_batch_size, lstm_hidden_units[-1])
                print(
                    "encoder_decoder_stacked_lstm_regression: rnn_decoder: state = {}"
                    .format(
                        state))  # tuple of final decoder c_state and h_state

                # Append the current decoder hidden state output to the outputs list
                outputs.append(
                    output
                )  # growing list eventually output_sequence_length long of shape = (current_batch_size, lstm_hidden_units[-1])
    ...
    if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
        ...
        unstacked_labels = tf.unstack(
            value=labels, num=params['output_sequence_length'], axis=1
        )  # list of output_sequence_length long of shape = (current_batch_size,)
        print("encoder_decoder_stacked_lstm_regression: unstacked_labels = {}".
              format(unstacked_labels))

        # Expand each 1-D label tensor back into a 2-D tensor
        expanded_unstacked_labels = [
            tf.expand_dims(input=tensor, axis=-1)
            for tensor in unstacked_labels
        ]  # list of output_sequence_length long of shape = (current_batch_size, 1)
        print(
            "encoder_decoder_stacked_lstm_regression: expanded_unstacked_labels = {}"
            .format(expanded_unstacked_labels))
        ...
        decoder_outputs, decoder_states = rnn_decoder(
            decoder_inputs=expanded_unstacked_labels,
        ...
        fake_labels = [
            tf.zeros(shape=[current_batch_size, 1])
            for _ in range(params['output_sequence_length'])
        ]
        print(
            "encoder_decoder_stacked_lstm_regression: fake_labels = {}".format(
                fake_labels))
        ...
        decoder_outputs, decoder_states = rnn_decoder(
            ...
            inference=True)
    print(
        "encoder_decoder_stacked_lstm_regression: decoder_outputs = {}".format(
            decoder_outputs)
    )  # list output_sequence_length long of shape = (current_batch_size, lstm_hidden_units[-1])
    print(
        "encoder_decoder_stacked_lstm_regression: decoder_states = {}".format(
            decoder_states))  # tuple of final decoder c_state and h_state

    # Stack together the list of decoder output tensors into one
    stacked_decoder_outputs = tf.stack(
        values=decoder_outputs, axis=0
    )  # shape = (current_batch_size * output_sequence_length, lstm_hidden_units[-1])
    print(
        "encoder_decoder_stacked_lstm_regression: stacked_decoder_outputs = {}"
    ...
    print("encoder_decoder_stacked_lstm_regression: network = {}".format(
        network))
    ...
    with tf.variable_scope(name_or_scope="dnn", reuse=tf.AUTO_REUSE):
        ...
        for units in params['dnn_hidden_units']:
            network = tf.layers.dense(
                inputs=network, units=units, activation=tf.nn.relu
            )  # shape = (current_batch_size * output_sequence_length, dnn_hidden_units[i])
            print(
                "encoder_decoder_stacked_lstm_regression: network = {}, units = {}"
                .format(network, units))

        # Connect the final hidden layer to a dense layer with no activation to get the logits
        logits = tf.layers.dense(
            inputs=network, units=1, activation=None
        )  # shape = (current_batch_size * output_sequence_length, 1)
    ...
    predictions = tf.reshape(
        tensor=logits,
        shape=[current_batch_size, params['output_sequence_length']
               ])  # shape = (current_batch_size, output_sequence_length)
...
def serving_input_fn(input_sequence_length, reverse_sequence):
    ...
    features = {
        key: fix_shape_and_type_for_serving(placeholder=tensor)
    ...
    features = {
        key: get_shape_and_set_modified_shape_2D(
            tensor=tensor, additional_dimension_sizes=[input_sequence_length])
    ...
    sequence_lengths = tf.tile(
        input=[input_sequence_length],
        multiples=[tf.shape(feature_placeholders["price"])[0]])
    print("serving_input_fn: sequence_lengths = {}".format(sequence_lengths))

    # Reversing the input sequence can often improve performance because it shortens the path from the decoder to the relevant parts of the encoder
    if reverse_sequence == True:
        features = {
            key: tf.reverse_sequence(input=tensor,
                                     seq_lengths=sequence_lengths,
                                     seq_axis=1,
                                     batch_axis=0)
            for key, tensor in features.items()
...
def train_and_evaluate(args):
    ...
    estimator = tf.estimator.Estimator(
        model_fn=getattr(
            sys.modules[__name__], args["model_fn_name"]
        ),  # finds within current module the model function based on passed string
        ...
        params={
            ...
            args["lstm_dropout_output_keep_probs"],
            "dnn_hidden_units":
            args["dnn_hidden_units"],
            "learning_rate":
            args['learning_rate']
        })
    ...
    train_spec = tf.estimator.TrainSpec(input_fn=read_dataset(
        ...
        batch_size=args['batch_size'] + args['input_sequence_length'] +
        args['horizon'] + args['output_sequence_length'] - 1,
        params=args),
    ...
    eval_spec = tf.estimator.EvalSpec(
        input_fn=read_dataset(filename=args['eval_file_pattern'],
                              ...
                              args['input_sequence_length'] + args['horizon'] +
                              args['output_sequence_length'] - 1,
                              params=args),
        steps=None,
