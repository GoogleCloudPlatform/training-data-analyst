...
def build_estimator(model_dir, nbuckets, hidden_units):
  ...
  """
    ...
    # Input columns
    (dayofweek, hourofday, plat, plon, dlat, dlon, pcount, latdiff, londiff,
     euclidean) = INPUT_COLUMNS
    ...
    # Feature cross
    ploc = tf.feature_column.crossed_column([b_plat, b_plon],
                                            nbuckets * nbuckets)
    dloc = tf.feature_column.crossed_column([b_dlat, b_dlon],
                                            nbuckets * nbuckets)
...
def serving_input_fn():
    feature_placeholders = {
        ...
        for column in INPUT_COLUMNS[2:7]
    }
...
def train_and_evaluate(args):
    ...
    estimator = build_estimator(args['output_dir'], args['nbuckets'],
...
def generate_tfrecord_input_fn(data_paths,
                               num_epochs=None,
                               batch_size=512,
                               mode=tf.estimator.ModeKeys.TRAIN):
    def get_input_features():
        ...
        if mode != tf.estimator.ModeKeys.INFER:
            input_schema[LABEL_COLUMN] = tf.FixedLenFeature(shape=[1],
                                                            dtype=tf.float32,
                                                            default_value=0.0)
        for name in ['dayofweek', 'key']:
            input_schema[name] = tf.FixedLenFeature(shape=[1],
                                                    dtype=tf.string,
                                                    default_value='null')
        ...
        keys, features = tf.contrib.learn.io.read_keyed_batch_features(
            ...
            num_epochs=(1
                        if mode == tf.estimator.ModeKeys.EVAL else num_epochs))
