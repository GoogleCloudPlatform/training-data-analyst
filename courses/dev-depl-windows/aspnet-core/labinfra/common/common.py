...
def AutoName(base, resource, *args):
    ...
    auto_name = '%s-%s' % (base,
                           '-'.join(list(args) + [default.AKA[resource]]))
...
def GlobalComputeLink(project, collection, name):
    return ''.join([
        default.COMPUTE_URL_BASE, 'projects/', project, '/global/', collection,
        '/', name
    ])
...
def LocalComputeLink(project, zone, key, value):
    return ''.join([
        default.COMPUTE_URL_BASE, 'projects/', project, '/zones/', zone, '/',
        key, '/', value
    ])
...
def MakeSubnetworkComputeLink(context, key):
    ...
    return ''.join([
        default.COMPUTE_URL_BASE, 'projects/', project, '/regions/', region,
        '/subnetworks/', value
    ])
...
def IsComputeLink(name):
    return (name.startswith(default.COMPUTE_URL_BASE)
            or name.startswith(default.REFERENCE_PREFIX))
...
def TakeZoneOut(properties):
    ...
    if default.BOOTDISK in properties:
        properties[default.BOOTDISK] = _CleanZoneUrl(
            properties[default.BOOTDISK])
    if default.DISKS in properties:
        for disk in properties[default.DISKS]:
            ...
            if default.DISKTYPE in disk_init:
                disk_init[default.DISKTYPE] = _CleanZoneUrl(
