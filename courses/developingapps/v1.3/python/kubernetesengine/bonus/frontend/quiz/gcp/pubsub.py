...
answer_topic_path = publisher.topic_path(project_id, 'answers')
...
"""
Bonus: Publishes answer info 
- jsonify answer object
- encode as bytestring
- publish message
- return result
"""
def publish_answer(answer):
    payload = json.dumps(answer, indent=2, sort_keys=True)
    data = payload.encode('utf-8')
    future = publisher.publish(answer_topic_path, data=data)
    return future.result()


"""pull_answer

Starts pulling messages from answer subscription
- receive callback function from calling module
- initiate the pull providing the callback function
"""
