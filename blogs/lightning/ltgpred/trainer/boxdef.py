...
class BoxDef(object):
    ...
    def get_prediction_grid_centers(self, ref):
        ...
        cy, cx = np.meshgrid(
            np.arange(neighborhood_size, ref.shape[0] - neighborhood_size,
                      self.stride),
            np.arange(neighborhood_size, ref.shape[1] - neighborhood_size,
                      self.stride))
        cy = cy.ravel()
        cx = cx.ravel()
        return zip(cy, cx)
    ...
    def rawdata_input_fn(self, ref, ltg, griddef, ltgfcst=None):
        ...
        for cy, cx in self.get_prediction_grid_centers(ref):
            ...
            interesting = np.sum(
                ltg[cy - self.train_patch_radius:cy + self.train_patch_radius +
                    1, cx - self.train_patch_radius:cx +
                    self.train_patch_radius + 1]) > 0.5
            if interesting:
                label = (np.sum(ltgfcst[cy - self.label_patch_radius:cy +
                                        self.label_patch_radius + 1, cx -
                                        self.label_patch_radius:cx +
                                        self.label_patch_radius + 1]) > 0.5
                ...
                example = {
                    ...
                    ltg[cy][cx],
                    'ref_smallbox':
                    ref[cy - self.label_patch_radius:cy +
                        self.label_patch_radius + 1, cx -
                        self.label_patch_radius:cx + self.label_patch_radius +
                        1],
                    'ref_bigbox':
                    ref[cy - self.train_patch_radius:cy +
                        self.train_patch_radius + 1, cx -
                        self.train_patch_radius:cx + self.train_patch_radius +
                        1],
                    'ltg_smallbox':
                    ltg[cy - self.label_patch_radius:cy +
                        self.label_patch_radius + 1, cx -
                        self.label_patch_radius:cx + self.label_patch_radius +
                        1],
                    'ltg_bigbox':
                    ltg[cy - self.train_patch_radius:cy +
                        self.train_patch_radius + 1, cx -
                        self.train_patch_radius:cx + self.train_patch_radius +
                        1],
                    'has_ltg':
