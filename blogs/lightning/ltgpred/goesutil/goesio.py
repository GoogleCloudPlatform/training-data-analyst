...
@retry(wait_exponential_multiplier=RETRY_MIN_MSECS,
...
def copy_togcs(localfile, bucket_name, blob_name):
    ...
    logging.info('%s uploaded to gs://%s/%s', localfile, bucket_name,
                 blob_name)
    return blob
...
def get_ir_blob_paths(year, day, hour, channel='C14'):
    ir_blob_paths = list_gcs_paths(
        GOES_PUBLIC_BUCKET,
        'ABI-L1b-RadC/{}/{:03d}/{:02d}/'.format(year, day, hour), channel)
...
def get_ltg_blob_paths(dt, timespan_minutes):
  ...
  """
    ...
    ltg_blobs = list_gcs_paths(
        GOES_PUBLIC_BUCKET,
        'GLM-L2-LCFA/{}/{:03d}/{:02d}/'.format(dt.year,
                                               dt.timetuple().tm_yday,
                                               dt.hour), None)
    if startdt.hour != dt.hour:
        assert timespan_minutes < 120, (
            'this code will work only for 1-2 hours of '
            'data')
        year, day, hour = startdt.year, startdt.timetuple(
        ).tm_yday, startdt.hour
        ltg_blobs.extend(
            list_gcs_paths(
                GOES_PUBLIC_BUCKET,
                'GLM-L2-LCFA/{}/{:03d}/{:02d}/'.format(year, day, hour), None))
...
if __name__ == '__main__':
    ...
    ltg = create_ltg_grid(get_ltg_blob_paths(irdt, timespan_minutes=5), conus,
                          3)
    print(ltg)
