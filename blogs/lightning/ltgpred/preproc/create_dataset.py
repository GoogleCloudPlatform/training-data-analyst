...
def generate_hours(startdate: str, enddate: str, starthour: int, endhour: int,
                   is_train: bool):
    ...
    if endyear == startyear:
        yield from _generate_hours(starthour, endhour, startday, endday,
                                   startyear, is_train)
    else:
        # for startyear, go from startday to day#365
        # FIXME: leap years?
        yield from _generate_hours(starthour, endhour, startday, 365,
                                   startyear, is_train)
        for y in range(startyear + 1, endyear):
            yield from _generate_hours(starthour, endhour, 1, 365, endyear,
                                       is_train)
        yield from _generate_hours(starthour, endhour, 1, endday, endyear,
                                   is_train)
...
def create_training_examples(ref, ltg, ltgfcst, griddef, boxdef, samplingfrac):
    ...
    for example in boxdef.rawdata_input_fn(ref, ltg, griddef, ltgfcst):
        ...
        should_write = (example['has_ltg'] or random.random() < samplingfrac)
...
def get_ir_blob_paths(hours_dict, max_per_hour=None):
    ...
    blob_paths = goesio.get_ir_blob_paths(hours_dict['year'],
                                          hours_dict['day'],
                                          hours_dict['hour'])
...
def create_record(ir_blob_path, griddef, boxdef, forecast_minutes,
    ...
    if ltg_blob_paths:
        ...
        ltg_blob_paths = goesio.get_ltg_blob_paths(
            irdt, timespan_minutes=ltg_validity_minutes)
        if ltg_blob_paths:
            ltgfcst = goesio.create_ltg_grid(ltg_blob_paths, griddef,
                                             influence_km)

            # create examples
            for example in create_training_examples(ref, ltg, ltgfcst, griddef,
                                                    boxdef, sampling_frac):
                yield example
...
def run_job(options):  # pylint: disable=redefined-outer-name
    ...
    boxdef = bd.BoxDef(options['train_patch_radius'],
                       options['label_patch_radius'], options['stride'])
    ...
    with beam.Pipeline(options['runner'], options=opts) as p:
        for step in ['train', 'eval']:
            ...
            examples = (
                ...
                | '{}_hours'.format(step) >> beam.Create(
                    generate_hours(options['startday'], options['endday'],
                                   ...
                                   step == 'train'))
                | '{}_irblobs'.format(step) >> beam.FlatMap(
                    lambda x: get_ir_blob_paths(x, options['max_per_hour']))
                | '{}_examples'.format(step) >> beam.FlatMap(
                    lambda ir_blob_path:  # pylint: disable=g-long-lambda
                    create_record(ir_blob_path, griddef, boxdef, options[
                        'forecast_interval'], options['lightning_validity'],
            ...
            examples = (
                examples
                | '{}_reshuffleA'.format(step) >>
                beam.Map(lambda t: (random.randint(1, 1000), t))
            ...
            if step == 'train':
                _ = (
                    examples
                    | 'get_values' >>
                    beam.FlatMap(lambda x: [(f, x[f]) for f in ['ref', 'ltg']])
                    | 'compute_stats' >> beam.CombinePerKey(MeanStddev())
                    | 'write_stats' >> beam.io.Write(
                        beam.io.WriteToText(os.path.join(
                            options['outdir'], 'stats'),
                                            num_shards=1)))
            ...
            _ = (examples
                 ...
                 | '{}_writecsv'.format(step) >> beam.io.Write(
                     beam.io.WriteToText(
                         os.path.join(options['outdir'], 'csv', step))))

            # write out tfrecords
            _ = (examples
                 | '{}_tfrecords'.format(step) >>
                 beam.Map(lambda x: x['tfrecord'])
                 | '{}_writetfr'.format(step) >>
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_patch_radius',
                        type=int,
                        default=2,
    ...
    parser.add_argument('--stride',
                        type=int,
                        default=4,
                        help='patch translation')
    parser.add_argument('--latlonres',
                        type=float,
                        default=0.02,
    ...
    parser.add_argument('--endhour',
                        type=int,
                        default=23,
                        help='end hour of day (0-23)')
    parser.add_argument('--max_per_hour',
                        type=int,
                        default=1,
                        help='how many IR records per hour')
    parser.add_argument('--forecast_interval',
                        type=int,
                        default=30,
                        help='how far ahead to forecast (minutes)')
    parser.add_argument('--lightning_validity',
                        type=int,
                        default=15,
    ...
    if not options['project']:
        ...
        print('Launching Dataflow job {} ... hang on'.format(
            options['job_name']))
        try:
