...
def prepare_steaming_source(project_id, pubsub_topic, pubsub_subscription):
    ...
    if subscription.exists():
        print(
            'Deleting pub/sub subscription {}...'.format(pubsub_subscription))
        subscription.delete()
...
def prepare_steaming_sink(project_id, bq_dataset, bq_table):
    ...
    schema = [
        bigquery.SchemaField(field_name, data_type)
...
def run_pipeline(inference_type,
                 project,
                 pubsub_topic,
                 pubsub_subscription,
                 bq_dataset,
    ...
    pubsub_subscription_url = "projects/{}/subscriptions/{}".format(
        project, pubsub_subscription)
    ...
    (pipeline
     | 'Read from PubSub' >> beam.io.ReadStringsFromPubSub(
         subscription=pubsub_subscription_url, id_label="source_id")
     | 'Estimate Targets - {}'.format(inference_type) >>
     beam.FlatMap(lambda message: estimate(message, inference_type))
     | 'Write to BigQuery' >> beam.io.WriteToBigQuery(
         project=project, dataset=bq_dataset, table=bq_table))
