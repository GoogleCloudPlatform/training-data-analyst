...
def createJson(line):
    ...
    rowid = uuid.uuid4().int & (
        1 << 63) - 1  # make up a 64-bit rowid since this data doesn't have it
    for name in [
            'CARRIER', 'ORIGIN', 'DEST', 'DEP_DELAY', 'ARR_DELAY', 'CANCELLED'
    ]:
        try:
            value = featdict[name]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=
        'Convert CSV of flights data to the JSON format expected by Inference API'
    )
