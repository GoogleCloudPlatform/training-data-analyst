...
def input_fn(indf):
    ...
    df["plurality"] = df["plurality"].astype(
        pd.api.types.CategoricalDtype(
            categories=["Single", "Multiple", "1", "2", "3", "4", "5"]))
    df["is_male"] = df["is_male"].astype(
        pd.api.types.CategoricalDtype(categories=["Unknown", "false", "true"]))
    # features, label
    label = df['label']
    del df['label']
...
def save_model(estimator, gcspath, name):
    ...
    model_path = os.path.join(
        gcspath,
        datetime.datetime.now().strftime('export_%Y%m%d_%H%M%S'), model)
