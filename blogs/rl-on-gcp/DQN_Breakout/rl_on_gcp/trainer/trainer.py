...
def _run(args):
  ...
  """
    with tf.Session() as sess:
        ...
        env = StackFrameEnv(env, args[0].frame_stack, args[0].img_height,
                            args[0].img_width)
        state = env.reset()
        ...
        agent = Agent(height=args[0].img_height,
        ...
        target_agent = Agent(height=args[0].img_height,
                             width=args[0].img_width,
                             actions=env.action_space.n,
                             channels=args[0].frame_stack)
        ...
        if args[0].mode != 'Train':
            ...
            try:
                ...
                print(
                    '{} is not a valid .h5 model.'.format(trained_model_path))
        eta = args[0].init_eta
        ...
        for curr_step in range(args[0].steps):
            if (episode_number % DISPLAY_RESULTS == 0
                    and episode_run) or args[0].mode != 'Train':
                episode_reward = agent.play(env, model_dir, args[0].mode)
                print(
                    'CURRENT STEP: {}, EPISODE_NUMBER: {}, EPISODE REWARD: {},'
                    'EPSILON: {}'.format(curr_step, episode_number,
                                         episode_reward, eta))
                episode_run = False
            ...
            if args[0].mode == 'Train':
                ...
                ready_to_update_model = curr_step > args[
                    0].start_train and len(Buffer.buffer) > Buffer.min_size
                if ready_to_update_model:
                    exp_state, exp_next_state, exp_reward, exp_action, exp_done = Buffer.sample_experiences(
                        args[0].batch_size)
                    agent.batch_train(exp_state, exp_next_state, exp_reward,
                                      exp_action, exp_done, target_network,
                                      args[0].Q_learning)
                    if curr_step % args[0].update_target == 0:
                        target_network.set_weights(agent.model.get_weights())
                    if curr_step % (SAVE_STEPS_FACTOR * args[0].update_target
                                    ) == 0 and args[0].save_model:
                        print('SAVING MODEL AT STEP: ', curr_step)
                        models.save_model(
                            agent.model, model_dir + 'model_' +
                            str(episode_number) + '_.h5')
                #Resets state
...
def _parse_arguments(argv):
    ...
    parser.add_argument(
        ...
        type=str,
        default="Train")
    parser.add_argument('--init_eta',
                        help='Epsilon for taking actions',
                        type=float,
                        default=0.95)
    ...
    parser.add_argument('--learning_rate',
                        ...
                        type=float,
                        default=0.00025)
    parser.add_argument('--Q_learning',
