...
def build_fully_connected(X,
                          n_units=100,
                          activation=tf.keras.activations.relu,
                          initialization=he_init,
...
def dnn_custom_estimator(features, labels, mode, params):
    ...
    tf.summary.scalar('accuracy',
                      accuracy[1])  # for visualizing in TensorBoard
...
def read_mnist(X, y, num_epochs=None, batch_size=550, shuffle=False):
    ...
    return tf.estimator.inputs.numpy_input_fn(x=feature_dict,
                                              y=y.astype(np.int32),
...
train_input_fn = read_mnist(mnist.train.images,
                            mnist.train.labels,
                            num_epochs=None,
                            batch_size=BATCH_SIZE,
                            shuffle=True)

eval_input_fn = read_mnist(mnist.test.images,
                           mnist.test.labels,
                           num_epochs=1,
                           batch_size=BATCH_SIZE,
                           shuffle=False)
...
def train_and_evaluate(output_dir):
    print(HIDDEN_UNITS)
    features = [
        tf.feature_column.numeric_column(key='image_data', shape=(28 * 28))
    ]
