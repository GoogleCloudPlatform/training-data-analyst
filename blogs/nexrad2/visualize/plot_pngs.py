...
def plot_data(infilename, outpng, maxrange):
    ...
    display.plot('reflectivity',
                 1,
                 ax=ax,
                 title='Reflectivity second',
                 colorbar_label='')
    display.set_limits((-maxrange, maxrange), (-maxrange, maxrange), ax=ax)

    ax = fig.add_subplot(224)
    display.plot('cross_correlation_ratio',
                 0,
                 ax=ax,
...
if __name__ == '__main__':
    ...
    parser.add_argument('range',
                        help="maximum range to plot. default=300",
                        default=300,
                        type=float,
                        nargs='?')
    args = parser.parse_args()

    print "Plotting {} into {} upto {} km".format(args.nexrad, args.png,
                                                  args.range)
