...
public class APPipeline {
  ...
  public static interface MyOptions extends DataflowPipelineOptions {
    ...
    @Default.String("KTUS,KPHX,YUMA,KFLG")
    String getRadars();
  ...
  public static void main(String[] args) {
    ...
    // GCP will scale out the processing to find the AP
    PCollection<APDetector.AnomalousPropagation> ap = p//
        .apply("archives", Create.of(getArchivesToRead(options))) //
          ...
          public void processElement(ProcessContext c) throws Exception {
            ...
            try (GcsNexradL2Read hourly = new GcsNexradL2Read(tarFile)) {
              for (RadialDatasetSweep volume : hourly.getVolumeScans()) {
                for (AnomalousPropagation ap : APDetector.findAP(volume)) {
                  c.output(ap);
                }
              }
            }
    ...
    ap.apply("ByRadar", ParDo.of(new DoFn<AnomalousPropagation, KV<String, Integer>>() {
      ...
      public void processElement(ProcessContext c) throws Exception {
        ...
        .apply("to", TextIO.Write.to(options.getOutput() + "totalByRadar")//
  ...
  private static List<String> getArchivesToRead(MyOptions options) {
    // get all the archives to read
    List<String> tars = new ArrayList<>();
    for (String radar : options.getRadars().split(",")) {
      for (int year : YEARS) {
        for (int month : MONTHS) {
          tars.addAll(GcsNexradL2List.getFiles(radar, year, month));
        }
      }
    }
    return tars;
