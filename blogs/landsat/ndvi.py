    ...
    with LandsatReader(gs_baseurl, band1, '.') as red_ds, \
       LandsatReader(gs_baseurl, band2, '.') as nir_ds :
        ...
        tmpfilename = os.path.join(
            tempfile.gettempdir(),
            '{0}_ndvi.TIF'.format(os.path.basename(gs_baseurl)))
        driver = gdal.GetDriverByName('GTiff')
        outds = driver.Create(tmpfilename, red_ds.RasterXSize,
                              red_ds.RasterYSize, 1, outdtype)
        ...
        for line in xrange(0, red.YSize):
            red_data = struct.unpack(
                packformat,
                red.ReadRaster(0, line, red.XSize, 1, red.XSize, 1,
                               gdal.GDT_Float32))
            nir_data = struct.unpack(
                packformat,
                nir.ReadRaster(0, line, nir.XSize, 1, nir.XSize, 1,
                               gdal.GDT_Float32))
            ndvi_scaled = np.array([255] * red.XSize,
                                   dtype=np.float32)  # mask value
            ...
            for i in xrange(0, len(red_data)):
                ...
                if ndvi_denom != 0:
                    ...
                    ndvi_scaled[i] = int(0.5 + 100 *
                                         ndvi) if ndvi > 0 else 0  # 0-100
            outline = struct.pack(packformat, *ndvi_scaled)
            outds.GetRasterBand(1).WriteRaster(0,
                                               line,
                                               red.XSize,
                                               1,
                                               outline,
        ...
        outfilename = os.path.join(
            outdir, '{0}_ndvi.TIF'.format(os.path.basename(gs_baseurl)))
...
if __name__ == '__main__':
    computeNdvi(
        'gs://gcp-public-data-landsat/LE07/PRE/198/057/LE71980572015351ASN00',
        'gs://cloud-training-demos/landsat/', 'LANDSAT_7')  # cape palmas
    computeNdvi(
        'gs://gcp-public-data-landsat/LC08/PRE/153/075/LC81530752015348LGN00',
        'gs://cloud-training-demos/landsat/', 'LANDSAT_8')  # reunion
