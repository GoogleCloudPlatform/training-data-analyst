...
def cnn_batch_norm_model(img, mode, hparams):
    ...
    c2 = tf.layers.conv2d(p1,
                          ...
                          strides=1,
                          padding='same',
                          activation=tf.nn.relu)  #?x14x14x20
    ...
    if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
        loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(logits=ylogits,
                                                       labels=labels))
        evalmetrics = {
            'accuracy': tf.metrics.accuracy(classes, tf.argmax(labels, 1))
        }
...
def train_and_evaluate(output_dir, hparams):
    ...
    train_images = train_images.astype(
        dtype=np.float32, copy=False).reshape(train_images.shape + (1, ))
    test_images = test_images.astype(
        dtype=np.float32, copy=False).reshape(test_images.shape + (1, ))
