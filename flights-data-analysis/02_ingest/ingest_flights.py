...
def download(YEAR, MONTH):
   ...
   '''
   print 'Requesting data for {}-{}-*'.format(YEAR, MONTH)
   ...
   filename = "{}{}.zip".format(YEAR, MONTH)
...
def zip_to_csv(filename):
   zip_ref = zipfile.ZipFile(filename, 'r')
   zip_ref.extractall()
   csvfile = zip_ref.namelist()[0]
   zip_ref.close()
   print "Extracted {} ... removing {}".format(csvfile, filename)
   os.remove(filename)
   return csvfile

def remove_quotes_comma(csvfile, year, month):
   '''
     returns output_csv_file, number_of_lines
   '''
   N = 0
   outfile = '{}{}.csv'.format(year, month)
   with open(csvfile, 'r') as infp:
     with open(outfile, 'w') as outfp:
        for line in infp:
           ...
           outfp.write('\n')
           N = N + 1
   print 'Cleaned up {} lines and wrote {} ... removing {}'.format(N, outfile, csvfile)
   os.remove(csvfile)
   return (outfile, N)

def upload(csvfile, gcslocation, domain = None):
   subprocess.check_call(['gsutil', 'cp', csvfile, gcslocation])
   print 'Uploaded {} ... removing {}'.format(gcslocation, csvfile)
   os.remove(csvfile)
   if domain is not None:
      subprocess.check_call(['gsutil', 'acl', 'ch', '-g', '{}:R'.format(domain), gcslocation]) 
...
def ingest(year, month, bucket):
   ...
   '''
   zip = download(year, month)
   bts_csv = zip_to_csv(zip)
   csvfile, numlines = remove_quotes_comma(bts_csv, year, month)
   if numlines > 1:
      upload(csvfile, 'gs://{}/flights/{}'.format(bucket, csvfile), None) # 'google.com')
      return True
   else:
      print 'Requested data is not available'
      return False

def next_month(bucket):
   '''
     Finds which months are on GCS, and returns next year,month to download
   '''
   files = subprocess.check_output(['gsutil', 'ls', 'gs://{}/flights/'.format(bucket)])
   files = files.split('\n')
   files = [f for f in files if 'csv' in f] # the csv files in that location
   lastfile = os.path.basename(files[-1])
   print 'The latest file on GCS is {}'.format(lastfile)
   year = lastfile[:4]
   month = lastfile[4:6]
   dt = datetime.datetime(int(year), int(month), 15) # 15th of month
   dt = dt + datetime.timedelta(30) # will always go to next month
   print 'The next month is {}'.format(dt)
...
if __name__ == '__main__':
   ...
   ret = ingest(year, month, args.bucket)
   if ret:
      print 'Success'
   else:
