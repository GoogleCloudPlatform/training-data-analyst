...
def feature_cross_latlon(plat, plon, name, NUMBUCKETS):
    ...
    dplatlon = tf.squeeze(tf.one_hot(dplatlon_index, depth, axis=-1, name='{0}_location'.format(name)))
    return dplatlon_index, dplatlon

def print_shape(t, name):
...
def create_inputs(metadata, input_data, hyperparams):
  with tf.name_scope('inputs'):
    ...
    NUMBUCKETS = hyperparams['number_buckets']
    pickup_index, pickup = feature_cross_latlon(plat, plon, 'pickup', NUMBUCKETS)
    dropoff_index, dropoff = feature_cross_latlon(dlat, dlon, 'dropoff', NUMBUCKETS)
    # pickupdropoff is [batchsize, nbuckets**4]
    pickupdropoff = tf.squeeze(tf.one_hot(tf.mul(pickup_index, dropoff_index), depth=NUMBUCKETS**4, axis=-1))
    latdist = tf.abs(tf.sub(plat, dlat))
    londist = tf.abs(tf.sub(plon, dlon))

    print_shape(plat, 'plat')
    print_shape(latdist, 'latdist')
    print_shape(pickup, 'pickup')
    print_shape(pickupdropoff, 'pickupdropoff')
...
def inference_None(inputs, metadata, hyperparams):
  # error about last layer being None when writing the graph (okay in training)
  NUMBUCKETS = hyperparams['number_buckets']
  input_size = 6 + (NUMBUCKETS**2) * 2 + (NUMBUCKETS**4) + 4
  output_size = metadata.features['fare_amount']['size'] # outputs
